...
function analysisMarkup(data, mode) {
  ...
  return `<div class="results"><div class="score-row"><div class="score-caption">${mode === 'recruiter' ? 'RECRUITER REVIEW / הערכת מועמד' : 'SIMULATION READY'}<strong>${data.headline || 'ניתוח מוכן'}</strong></div><div class="score-ring" style="--score:${data.matchScore || 0}%"><b>${data.matchScore || 0}%</b></div></div><p class="result-summary">${data.summary || ''}</p><div class="result-columns"><div class="mini-panel"><h3>חוזקות</h3><ul>${list(data.strengths)}</ul></div><div class="mini-panel warning"><h3>פערים וסיכונים</h3><ul>${list(data.gaps)}</ul></div></div><div class="focus-block"><h3>נקודות לבדיקה</h3><ul>${list(data.focus)}</ul></div><div>${questions}</div><div class="tips-block"><h3>המלצות</h3><p>${(data.tips || []).join(' · ')}</p></div>${chatMarkup(mode)}</div>`;
}
function renderAnalysis(data, board, mode) {
  board.innerHTML = analysisMarkup(data, mode);
  board.querySelector('.chat-form').addEventListener('submit', event => sendChat(event, mode));
  setupSpeechToText(mode, board);
}
function createAnalysisLoadingPage(reportWindow) {
  const loadingHtml = `<!doctype html><html lang="he" dir="rtl"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>מכין ניתוח | Interview Orbit</title><style>html,body{margin:0;min-height:100%;background:#f4f0e8;color:#101719;font-family:"Times New Roman",Times,serif}body{min-height:100vh;display:grid;place-items:center}.loading-screen{text-align:center;padding:30px}.loading-screen h1{font-size:34px;margin:0 0 28px;font-weight:800}.loading-circle{width:180px;height:180px;margin:auto;border-radius:50%;display:grid;place-items:center;background:conic-gradient(#4c9490 0deg,#4c9490 0deg,#d8d5cd 0deg);position:relative;transition:background .12s linear}.loading-circle:after{content:"";position:absolute;inset:9px;border-radius:50%;background:#f4f0e8}.loading-percent{position:relative;z-index:1;font-size:42px;font-weight:800}.loading-note{margin-top:24px;color:#3f4a4d;font-size:22px;font-weight:800}.loading-circle.blink{animation:loading-blink .18s ease-in-out alternate 6}@keyframes loading-blink{from{filter:brightness(1)}to{filter:brightness(1.8);transform:scale(1.035)}}@media(prefers-reduced-motion:reduce){.loading-circle{transition:none}.loading-circle.blink{animation:none}}</style></head><body><main class="loading-screen"><h1>מיד הניתוח יעלה</h1><div class="loading-circle"><span class="loading-percent">0%</span></div><p class="loading-note">מנתח את ההתאמה למשרה...</p></main><script>let progress=0;let target=null;let ready=false;let finished=false;const circle=document.querySelector('.loading-circle');const percent=document.querySelector('.loading-percent');function paint(){percent.textContent=Math.round(progress)+'%';circle.style.background='conic-gradient(#4c9490 '+(progress*3.6)+'deg,#d8d5cd 0deg)'}function finish(){if(finished)return;finished=true;let flashes=0;const blink=()=>{circle.classList.add('blink');setTimeout(()=>{circle.classList.remove('blink');flashes+=1;if(flashes<3)setTimeout(blink,80);else setTimeout(()=>window.opener&&window.opener.postMessage({type:'analysis-loading-complete'},'*'),180)},180)};blink()}function tick(){if(target===null){progress=Math.min(86,progress+.34)}else{const distance=target-progress;if(Math.abs(distance)<.5){progress=target;paint();finish();return}progress+=Math.sign(distance)*Math.max(.18,Math.abs(distance)*.055)}paint();requestAnimationFrame(tick)}window.addEventListener('message',event=>{if(event.data&&event.data.type==='analysis-ready'){target=Math.max(0,Math.min(100,Number(event.data.score)||0));ready=true}});window.opener&&window.opener.postMessage({type:'analysis-loading-ready'},'*');paint();requestAnimationFrame(tick)</script></body></html>`;
  const loadingUrl = URL.createObjectURL(new Blob([loadingHtml], { type: 'text/html' }));
  reportWindow.location.replace(loadingUrl);
  setTimeout(() => URL.revokeObjectURL(loadingUrl), 120000);
}
function openAnalysisReport(data, reportWindow) {
  const cssUrl = `${location.origin}/styles.css?v=${Date.now()}`;
  const chatCssUrl = `${location.origin}/chat.css?v=${Date.now()}`;
  const portalCssUrl = `${location.origin}/portal.css?v=${Date.now()}`;
  const score = Number(data.matchScore) || 0;
  const scoreClass = score <= 50 ? 'report-score-low' : score <= 70 ? 'report-score-medium' : score <= 85 ? 'report-score-good' : 'report-score-high';
  const reportHtml = `<!doctype html><html lang="he" dir="rtl"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>ניתוח מועמד | Interview Orbit</title><link rel="stylesheet" href="${cssUrl}"><link rel="stylesheet" href="${chatCssUrl}"><link rel="stylesheet" href="${portalCssUrl}"></head><body class="report-page ${scoreClass}"><div class="app-shell report-shell"><header class="topbar"><a class="brand" href="#"><span class="brand-mark"><span></span><span></span><span></span></span><span>INTERVIEW <b>ORBIT</b></span></a><div class="report-actions"><span>RECRUITER REVIEW</span><button class="report-print" type="button" onclick="window.print()">ייצוא ל־PDF <span>↓</span></button></div></header><main><div class="report-heading"><span>RECRUITER REVIEW / הערכת מועמד</span><h1>ניתוח מועמד</h1></div><section class="report-content">${analysisMarkup(data, 'recruiter')}</section></main><footer><span>INTERVIEW ORBIT / RECRUITING WORKSPACE</span><span>CONFIDENTIAL</span></footer></div></body></html>`;
...
async function loadRecruiters() { try { const data = await request('/api/recruiters'); $('#recruitersList').innerHTML = data.recruiters.map(item => `<div class="recruiter-row"><strong>${item.username}</strong><small>נוצר/ה ${new Date(item.createdAt).toLocaleDateString('he-IL')}</small></div>`).join('') || '<p class="muted-copy">אין עדיין מנהלים מגייסים.</p>'; } catch (error) { $('#recruitersList').innerHTML = `<p class="form-error">${error.message}</p>`; } }
...
document.querySelectorAll('[data-view]').forEach(button => button.addEventListener('click', async () => { const view = button.dataset.view; if (view === 'candidate') { const invite = new URLSearchParams(location.search).get('invite'); if (invite) return loadInvite(invite); candidateJob = { title: 'ראיון כללי', description: 'סימולציית ראיון כללית לתפקיד שיבחר המועמד.' }; $('#candidateJobBanner').innerHTML = '<span>OPEN SIMULATION</span><strong>סימולציה עצמאית</strong><p>התחל/י עם קורות החיים שלך.</p>'; return showView('candidateView'); } $('#loginRole').value = view; $('#loginRoleLabel').textContent = view === 'admin' ? 'ADMIN' : 'RECRUITER'; $('#recruiterUsernameField').classList.toggle('hidden', view === 'admin'); $('#username').required = view === 'recruiter'; showView('loginView'); }));
document.querySelectorAll('[data-back]').forEach(button => button.addEventListener('click', () => showView(button.dataset.back)));
$('#loginForm').addEventListener('submit', async event => { event.preventDefault(); try { const role = $('#loginRole').value; const data = await request('/api/auth/login', { method: 'POST', body: JSON.stringify({ role, username: $('#username').value, password: $('#password').value }) }); accessToken = data.accessToken; $('#logoutButton').classList.remove('hidden'); $('#sessionLabel').textContent = role.toUpperCase(); $('#loginError').textContent = ''; showView(role === 'admin' ? 'adminView' : 'recruiterView'); if (role === 'admin') await loadRecruiters(); else await loadJobs($('#jobsList')); } catch (error) { $('#loginError').textContent = error.message; } });
$('#logoutButton').addEventListener('click', () => { accessToken = ''; $('#logoutButton').classList.add('hidden'); showView('landingView'); });
$('#newJobButton').addEventListener('click', () => $('#newJobPanel').classList.toggle('hidden'));
$('#recruiterJobForm').addEventListener('submit', async event => { event.preventDefault(); try { const data = await request('/api/jobs', { method: 'POST', body: JSON.stringify({ title: $('#recruiterJobTitle').value, description: $('#recruiterJobDescription').value }) }); $('#recruiterJobMessage').textContent = `המשרה נשמרה: ${data.job.title}`; event.target.reset(); $('#newJobPanel').classList.add('hidden'); await loadJobs($('#jobsList')); } catch (error) { $('#recruiterJobMessage').textContent = error.message; } });
$('#recruiterFormAdmin').addEventListener('submit', async event => { event.preventDefault(); try { await request('/api/recruiters', { method: 'POST', body: JSON.stringify({ username: $('#adminUsername').value, password: $('#adminRecruiterPassword').value }) }); $('#recruiterMessage').textContent = 'המגייס/ת נוסף/ה בהצלחה.'; event.target.reset(); await loadRecruiters(); } catch (error) { $('#recruiterMessage').textContent = error.message; } });
...
$('#recruiterForm').addEventListener('submit', async event => { event.preventDefault(); const reportWindow = window.open('', '_blank'); if (!reportWindow) { alert('יש לאפשר חלונות קופצים כדי לפתוח את דף הניתוח.'); return; } let loadingReady = false; let analysisData = null; let loadingComplete = false; const loadingListener = message => { if (message.source !== reportWindow || !message.data) return; if (message.data.type === 'analysis-loading-ready') { loadingReady = true; if (analysisData) reportWindow.postMessage({ type: 'analysis-ready', score: analysisData.matchScore }, '*'); } if (message.data.type === 'analysis-loading-complete' && analysisData && !loadingComplete) { loadingComplete = true; window.removeEventListener('message', loadingListener); openAnalysisReport(analysisData, reportWindow); } }; window.addEventListener('message', loadingListener); createAnalysisLoadingPage(reportWindow); const board = $('#recruiterBoard'); board.innerHTML = '<div class="loading-state"><div class="loading-pulse"></div><p>מכין הערכת מועמד...</p><span>GEMINI IS THINKING</span></div>'; try { const data = await request('/api/analyze', { method: 'POST', body: JSON.stringify({ role: $('#recruiterRole').value, jobDescription: $('#recruiterDescription').value, resume: recruiterPdf ? '' : $('#recruiterResume').value, resumePdf: recruiterPdf }) }); analysisData = data; if (loadingReady) reportWindow.postMessage({ type: 'analysis-ready', score: data.matchScore }, '*'); renderAnalysis(data, board, 'recruiter'); } catch (error) { window.removeEventListener('message', loadingListener); reportWindow.close(); board.innerHTML = `<div class="board-empty"><p class="form-error">${error.message}</p></div>`; } });
