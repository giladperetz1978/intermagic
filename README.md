...
- מגייס/ת נכנס/ת עם `RECRUITER_PASSWORD`, בוחר/ת משרה שמורה, יוצר/ת לינק הזמנה חדש, מעתיק/ה אותו ופותח/ת הודעת מייל מוכנה לשליחה, ואז מנתח/ת מועמד ומשתמש/ת בצ'אט הגיוס.
- אדמין נכנס/ת עם `ADMIN_PASSWORD` ומוסיף/ה משרות לספרייה.
- משרות נשמרות בשרת תחת `data/jobs.json`; לכל משרה נוצר לינק מועמד ייעודי.
